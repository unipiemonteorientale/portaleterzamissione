...
# INNER QUALITATIVA
$this->respond('GET', "/inner", function ($request, $response, $service, $app) {
    $session = getSession();
    $db = getDB();
...
$this->respond('GET', "/2/inner", function ($request, $response, $service, $app) {
    ...
    $session->smarty()->assign("domande", $domande);
    $session->smarty()->assign("sezioni", $sezioni);
    $session->smarty()->assign("versione", 2);
    $session->smarty()->assign("anni", array(2023, 2022, 2021, 2020));
    $session->smarty->display("reportistica-inner.tpl");
});
