 ...
 */
class M3Documentazione extends UIObject {
    protected $_record_list;
    protected $_tree_code;
    
    function __construct() {
        $this->_object_name = "M3_DOCUMENTAZIONE";
    ...
    public function display_list($options=array()) {
        ...
        $smarty->display("ui-list-allegati.tpl");
    }
    ...
    public function input($options=array()) {
        $session = getSession();
        $db = getDB();
        error_log("M3Documentazione::input ({$this->_object_name})");
        ...
        if (strlen($model_key_name) && strlen($model_key_value)) {
            $record = new Base("MISSIONE3_CONF_Documentazione", $model_key_name, $model_key_value);
        }
        else
            $record = new Base();
    ...
    public function store($options=array()) {
        $session = getSession();
        error_log("M3Documentazione::store ({$this->_object_name})");
    ...
    public function delete($options=array()) {
        error_log("M3Documentazione::delete ({$this->_object_name})");
