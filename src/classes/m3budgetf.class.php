 ...
 */
class M3BudgetF extends M3Risposta {
...
<ul>
    <li>contributo UPO &euro; {$schema["supplement2"]}</li>
    <li>contributo esterno pubblico o privato di natura istituzionale &euro; {$schema["supplement3"]}</li>
    <li>quota conto terzi &euro; {$schema["supplement4"]}</li>
    <li>quote di iscizione &euro; {$schema["supplement5"]}</li>
</ul>
EOT;
        }
        else {
...
<div class="ui segment container_domanda field" style=''>
    ...
    <div class="ui stackable five column grid">
        ...
        <div class="column field">
            <label>contributo esterno pubblico o privato di natura istituzionale</label>
            <div class="ui labeled input">
                <div class="ui label"><i class="icon euro"></i></div>
        ...
        <div class="column field">
            <label>quota conto terzi</label>
            <div class="ui labeled input">
                <div class="ui label"><i class="icon euro"></i></div>
                <input type="number" step="0.01" name="{$schema["pre"]}supplement4" value="{$schema["supplement4"]}" {$readonly}/>
            </div>
        </div>
        <div class="column field">
            <label>quote di iscizione</label>
            <div class="ui labeled input">
                <div class="ui label"><i class="icon euro"></i></div>
                <input type="number" step="0.01" name="{$schema["pre"]}supplement5" value="{$schema["supplement5"]}" {$readonly}/>
            </div>
        </div>
    </div>
EOT;
    ...
    public function input($options=array()) {
        ...
        $budget_1 = $this->get("supplement2");
        $budget_2 = $this->get("supplement3");
        $budget_3 = $this->get("supplement4");
        $budget_4 = $this->get("supplement5");
        
        if ($budget_totale != $budget_1 + $budget_2 + $budget_3 + $budget_4) {
            $result = new Result();
            $result->setResult(false);
            $result->setCode("KO");
            $result->setDescription("Il budget totale non corrisponde alla somma dei singoli budget.");
